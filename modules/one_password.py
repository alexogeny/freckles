...
class OnePassword(object):
  ...
  def runner(self, command: str, internal: str=None, expected: int=0, ignore: bool=False):
    result = None
  ...
  def login_token(self):
    ...
    if result == []:
      ...
      self.token = ''.join(self.run_return(
  ...
  def check_login(self):
    try:
      rc, _, _, _ = self.runner('op account get', ignore=True, internal='is_logged_in')
      self.logged_in = rc == 0
      if not self.logged_in: self.login_token()
    except OSError as e:
  ...
  def get(self):
    ...
    try:
      ...
      module.fail_json(msg='Path should be in the format: vault.item.section.name')

    vault, item, section, name = self.path.split('.')

    try:
      _, _, _, result = self.runner(
        f'op read op://{vault}/{item}/{section}/{name} --account {self.sub}.1password.com --session "{self.token}"', internal='get'
      )
...
def main():
  ...
  module = AnsibleModule(
    argument_spec=dict(
      login_details=dict(type='dict', options=dict(
        email=dict(type='str'),
